...
static BackingImage *direct_allocateBackingImage_single(NVDriver *drv, NVSurface *surface, bool importedGpuCopy) {
...
static BackingImage *direct_allocateBackingImage(NVDriver *drv, NVSurface *surface, bool importedGpuCopy) {
    if (drv->descriptorMode == DESCRIPTOR_MODE_SINGLE && !isRgbSurfaceFourcc((uint32_t) surface->fourcc)) {
        return direct_allocateBackingImage_single(drv, surface, importedGpuCopy);
    }
